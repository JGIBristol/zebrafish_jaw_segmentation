...
\begin{tikzpicture}[node distance=1.8cm and 0.5cm]
	...
	\node (dec3concat) [block, below=0.5cm of att3] {$64\times20^3$};
	...
	\node (dec2concat) [block, below=0.5cm of att2] {$32\times40^3$};
	...
	\node (dec1concat) [block, below=0.5cm of att1] {$16\times80^3$};
	...
	\node (dec0concat) [block, below=0.5cm of att0] {$8\times160^3$};

	\node (tail) [block, right=1cm of merge0] {$8\times160^3$};

	\node (output_patch) [block, above=2.5cm of tail, fill=green!20, align=center] {Output\\$1\times160^3$};
	...
	\matrix[draw, below left=-2cm and 2cm of input_patch, column sep=0.4cm, row sep=0.3cm, font=\large] {
		...
		\node[concatmerge] at (0.3, 0) {+};\draw[concat](0.4, -0.2) -- +(0.6, 0.0); & \node[align=left, text width=4cm] {Concatenation}; \\
	};

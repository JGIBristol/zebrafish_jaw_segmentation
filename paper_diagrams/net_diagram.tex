...
\begin{tikzpicture}[node distance=1.8cm and 0.5cm]
	...
	% Decoder - skips
	\node (dec0) [concat, right=28.5cm of enc0] {$128\times10^3$};
	\node (dec1) [concat, right=22.5cm of enc1]   {$64\times20^3$};
	\node (dec2) [concat, right=16.5cm of  enc2]   {$32\times40^3$};
	\node (dec3) [concat, right=10.5cm of  enc3]   {$16\times80^3$};
	...
	\matrix[draw, below=3cm of dec0, column sep=0.4cm, row sep=0.3cm, font=\large] {
		...
		\draw[up]   (0,-0.3) -- +(0.6,0.0); & \node[align=left, text width=4cm] {Upsampling}; \\
